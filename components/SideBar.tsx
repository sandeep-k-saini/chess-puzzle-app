...
const Sidebar = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth <= 776;
      setIsMobile(mobile);
      setCollapsed(mobile);
    };
  ...
  return (
    <>
      ...
      {/* Mobile toggle button visible outside sidebar when collapsed */}
      {isMobile && collapsed && (
        <div
          style={{
            position: "fixed",
        ...
        >
          <img src="./Images/Group 2085664720.svg" alt="Toggle Sidebar" style={{ width: 16, height: 16 }} />
        </div>
      )}
    </>
  );
